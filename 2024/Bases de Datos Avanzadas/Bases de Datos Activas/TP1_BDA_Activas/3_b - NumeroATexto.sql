CREATE OR REPLACE PROCEDURE sp_numero_letras(
    IN numero numeric,
    OUT resultado text
) LANGUAGE plpgsql AS $$
DECLARE
    ...
    lnFraccion INTEGER;
    lnSw INTEGER;
    lcFraccionCadena TEXT;
BEGIN
    lnEntero := FLOOR(numero)::INTEGER; --Obtenemos la parte Entera
    lnFraccion := ROUND(((numero - lnEntero) * 100))::INTEGER; --Obtenemos la Fraccion del Monto
    lcRetorno := '';
    lnTerna := 1;
    
    -- Convertir la parte entera a texto
    IF lnEntero > 0 THEN
        lnSw := LENGTH(CAST(lnEntero AS TEXT));
    ...
    IF lnFraccion > 0 THEN
        lnTerna := 1;
        lcFraccionCadena := '';
        WHILE lnFraccion > 0 LOOP
            lcCadena := '';
            lnUnidades := lnFraccion % 10;
            lnFraccion := CAST(lnFraccion / 10 AS INTEGER);
            lnDecenas := lnFraccion % 10;
            lnFraccion := CAST(lnFraccion / 10 AS INTEGER);
            lnCentenas := lnFraccion % 10;
            lnFraccion := CAST(lnFraccion / 10 AS INTEGER);
            ...
            SELECT CASE
				...
				WHEN lnDecenas = 9 AND lnUnidades = 0 THEN 'NOVENTA ' || lcCadena 
				WHEN lnDecenas = 9 AND lnUnidades <> 0 THEN 'NOVENTA Y ' || lcCadena 
				ELSE lcCadena END INTO lcCadena;
	        SELECT CASE
	            WHEN lnCentenas = 1 AND lnUnidades = 0 AND lnDecenas = 0 THEN 'CIEN ' || lcCadena
	            WHEN lnCentenas = 1 AND NOT (lnUnidades = 0 AND lnDecenas = 0) THEN 'CIENTO ' || lcCadena
	            WHEN lnCentenas = 2 THEN 'DOSCIENTOS ' || lcCadena
	        ...
	        lcFraccionCadena := lcCadena || lcFraccionCadena;
	        lnTerna := lnTerna + 1;
	    END LOOP;
	    lcRetorno := RTRIM(lcRetorno) || ' PESOS CON ' || LTRIM(lcFraccionCadena) || 'CENTAVOS';
	ELSE
	    lcRetorno := RTRIM(lcRetorno) || ' PESOS CON ' || 'CERO CENTAVOS';
	END IF;
	resultado := lcRetorno;
END;
$$;
CALL sp_numero_letras(134.12, 'resultado');